...
\end{itemize}
Both conditions hold except with probability $\approx s_0 \, 2^{k_0}\, p/q$ ($\approx 2^{-116}$ for KoalaBear with $s_0 = 2$), so restarts are rare; each bad event is bounded in place in the proof sketch below.
...
Open questions: (i) quantify the failure probability of the no-accidental-pinning condition of the companion note \texttt{zk\_whir\_genericity.tex} (all its other restart conditions are bounded; a violation only triggers a restart, never a leak); (ii) the Fiat--Shamir and Merkle layers: the companion note proves the \emph{interactive} protocol perfectly zero-knowledge against the honest verifier.
