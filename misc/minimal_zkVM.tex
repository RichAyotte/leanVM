...
\title{LeanVM}
\author{draft 0.9.0 (Work In Progress)}
\date{}
\begin{document}
...
\subsection{Addressing modes}\label{sec:addressing-modes}
...
\textbf{Memory pulls.} For each operand $(\textsf{idx}, s) \in \{(\textsf{idx}_A, v_A),\, (\textsf{idx}_B, v_B),\, (\textsf{idx}_R, \textsf{res})\}$ and each $k \in [0, \extdeg)$:
\[
...
On every row $r$ of the extension table, $\textsf{len}_r \leq H - r$ (as an integer in $\{0, \dots, p-1\}$), where $H \leq 2^{22}$ is the table height. In particular, $\textsf{len}_r \leq H$.
\end{lemma}
...
\paragraph{No allocation pointer.} Cairo \cite{cairo} keeps a register $\textbf{ap}$ pointing to the next free memory cell. leanVM does not: the write-once memory (\Cref{sec:memory}) prevents any frame from overwriting previously-written data. The following convention is thus crucial: \textit{a function's internal logic should only depend on its inputs, not on where its frame happens to land}, so that the value of $\textbf{fp}_\text{callee}$ does not impact its execution.

\paragraph{The initial fp is likewise unconstrained.} For the same reason, the protocol does not pin the first row's $\textbf{fp}$ (the boundary checks of \Cref{sec:e2e-protocol} fix only the initial and final \textbf{pc}): program logic is not expected to depend on the exact value of $\textbf{fp}$, only on the memory values indexed relative to it.

\vspace{5mm}

The ISA makes it possible to use values of \textbf{fp} outside the range $[0, 2^{h_\textsc{memory}})$. As long as every memory access of the form $\textbf{m}[\textbf{fp} + x]$ (for some offset $x \in \Fp$) is in range, i.e. $\textbf{fp} + x < 2^{h_\textsc{memory}}$, the execution is valid. Yet, reasoning about such out-of-range values of $\textbf{fp}$ is tricky, and we thus recommend the following two programming conventions, that ensure $\textbf{fp}$ always stays in range $[0, 2^{h_\textsc{memory}})$:

\begin{itemize}
    \item The first instruction of the program should be $\texttt{ADD}[\texttt{imm}, 0,\;\; \texttt{imm}, 0,\;\; \texttt{mem}, 0]$, of semantics $\textbf{m}[\textbf{fp}] = 0$: the memory access at address $\textbf{fp}$ forces the initial $\textbf{fp} < 2^{h_\textsc{memory}}$ (\Cref{lem:indexed-lookup}).

    \item Whenever a JUMP updates $\textbf{fp}$, the program should guarantee that the new value is in range. The calling convention above does it for free:
    \begin{itemize}
        \item On a call, $\textbf{fp} \gets \textbf{m}[\textbf{fp} + x]$, where $\textbf{m}[\textbf{fp} + x] = \textbf{fp}_\text{callee}$ is hinted, hence arbitrary. But the callee frame was initialized with the DEREF $\textbf{m}[\textbf{m}[\textbf{fp} + x] + 0] \gets \textbf{pc}_\text{ret}$, whose memory access at address $\textbf{m}[\textbf{fp} + x]$ forces $\textbf{m}[\textbf{fp} + x] < 2^{h_\textsc{memory}}$ (\Cref{lem:indexed-lookup}).
        \item On a return, $\textbf{fp} \gets \textbf{m}[\textbf{fp} + 1]$ reads back what the caller wrote there, i.e. its own $\textbf{fp}$, in range by induction.
    \end{itemize}
\end{itemize}
