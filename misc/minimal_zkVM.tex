...
\begin{itemize}
    ...
    \item \emph{Round-0 sumcheck messages.} Restricted to the mask cells, each fresh coefficient is a linear combination of the per-class node values, with explicit public weights coming from the out-of-domain part of $\hat{W}_0$ (the input weight $\hat{w}$ also contributes cross-terms on $R_{\textsf{out}}$, but they are not needed; at $R_{\textsf{in}}$ positions they vanish, because the slice of $\hat{w}$ through an $R_{\textsf{in}}$ cell crosses only unclaimed positions). With a single out-of-domain point, these node weights satisfy $k_0 + 1$ exact linear dependencies, valid for \emph{every} choice of challenges: the sumcheck recursion and the terminal claim, restricted to node components. The node channel can then never mask all the values: this is why we require $s_0 \geq 2$. With two points, the two recursions have different coefficients, and the $2 k_0 + 2$ values (two answers, $2k_0$ fresh coefficients) become jointly independent combinations of the node values: proved in the companion note \texttt{zk\_whir\_genericity.tex}, under explicit per-slot conditions failing with probability $O(k_0 2^{k_0})/q$ (and verified numerically: rank $16/16$ at $k_0 = 7$, against $8/15$ for $s_0 = 1$). The node values are themselves fresh, by the previous two items: through $R_{\textsf{in}}$ for the data classes, through $R_{\textsf{out}}$ for the mask classes. Under \textsc{independence}, all the round-0 reveals are jointly uniform.
...
Open questions: (i) assemble the items above into a formal simulator theorem (the supporting rank and absorption lemmas are proven in the companion note \texttt{zk\_whir\_genericity.tex}); (ii) hiding of the Merkle roots and authentication paths.
