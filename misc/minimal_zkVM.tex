...
The protocol described so far is not zero-knowledge. A zero-knowledge variant is developed in the companion document \texttt{zk\_leanVM.tex}: a masked WHIR (two committed random masks folded into the data during the initial round, $s_0 \geq 2$ out-of-domain samples, and a prover-side restart rule), proven perfectly zero-knowledge against the honest verifier for the interactive opening, with all restart conditions bounded ($\Pr[\text{restart}] \approx 2^{-116}$); together with the masking strategies for the Logup-GKR and AIR sumchecks and for the column evaluations. The committed size roughly doubles; the verifier is unchanged. The Fiat--Shamir and Merkle layers remain open.
...
\begin{itemize}
    ...
    \item \textbf{Proof size:} opened leaves are sent without the zero suffix ($n_\text{eff}$ values instead of $2^{f_0^\text{whir}}$ per first-round query); the verifier re-appends the zeros before hashing.
\end{itemize}
