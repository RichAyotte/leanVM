...
\section{Zero-Knowledge (ideas only, not implemented, not proven)}\label{sec:zk}
...
\textbf{3. Restart rule.} After generating the proof, the prover checks the two conditions below; if either fails, it restarts with freshly sampled masks, which re-randomizes the entire transcript.
\begin{itemize}
...
\end{itemize}
Both conditions hold except with probability $\approx s_0 \, 2^{k_0}\, p/q$ ($\approx 2^{-117}$ for KoalaBear), so restarts are rare; each bad event is bounded in place in the proof sketch below.
...
\begin{itemize}
    ...
    \item \emph{Round-0 sumcheck messages.} On the mask cells, $\hat{W}_0$ reduces to its out-of-domain part: the lifted input weight $\hat{w}$ vanishes there (this is where $s_0 \geq 1$ is needed). A short computation then shows: restricted to one $u_s$, every fresh coefficient of $\hat{h}_1, \dots, \hat{h}_{k_0}$ is a scalar multiple of the evaluation of $\hat{u}_s$ at the out-of-domain nodes. So the sumcheck messages bring no new node: together with the out-of-domain answers, they are $2 k_0 + s_0$ public $\Fq$-linear combinations of the $2^{k_0} s_0$ values $\hat{u}_s(\pow(z^{2^{k_0}}, n - k_0))$, which the previous two items make jointly uniform. Masking holds as soon as this combination matrix has full rank, which fails with probability at most $(2k_0 + s_0)(k_0 + s_0)/q$ by Schwartz--Zippel, assuming its determinant is a nonzero polynomial (open question (i)). Under \textsc{independence}, all the round-0 reveals are jointly uniform.
    ...
    \]
    The random cells on the right are disjoint across $c$, with weights independent of $c$. If these $2^{k_0 - 1}$ weights span $\Fq$ over $\Fp$, which is exactly the \textsc{spread} condition, then every coefficient is uniform, independently: $\hat{f}_1$ is a uniformly random polynomial. (\textsc{spread} fails with probability at most $2/q$: \Cref{lem:spread} below.) One subtlety: round 0 already revealed evaluations of $R_{\textsf{out}}$, so conditioned on the view, $\hat{f}_1$ is no longer fully uniform. The precise claim: it is uniform among the polynomials $g$ satisfying the constraints the protocol checks anyway, namely $g(\pow(z_j, n - k_0)) = \fold(f_0, \vec{\alpha}_0)(z_j)$ for each queried $z_j$, and $g(\pow(z, n - k_0)) = y$ for each of the $s_1$ out-of-domain pairs $(z, y)$. Reason: a combination of round-0 values can match the weight pattern of $\hat{f}_1$'s coefficients only by assembling a whole fiber with the weights $\hat{eq}(\vec{\alpha}_0, \cdot)$, which produces exactly the first constraint; except with probability $O(\cdot)/q$, nothing else matches (open question (i)).
    \item \emph{Everything after round 0.} The oracles $f_1, \dots, f_{M-1}$, their fibers and out-of-domain answers, the remaining sumcheck messages, $\hat{f}_M$ and the final sumcheck are all deterministic functions of $\hat{f}_1$, the claims, and public challenges. The simulator samples $\hat{f}_1$ uniformly subject to the public constraints and runs the protocol honestly from there. Equivalently: the honest transcript is identically distributed for any two witnesses with the same claimed values.
\end{itemize}
...
\[
\frac{1}{q} \;+\; \binom{k_0 - 1}{k_0 - \extdeg + 1} \Big(\frac{p}{q}\Big)^{k_0 - \extdeg + 1} \;\leq\; \frac{2}{q} .
\]
\end{lemma}
...
Open questions: (i) prove the two remaining genericity claims: the determinant of the sumcheck combination matrix is a nonzero polynomial, and the ``nothing else matches'' step of the $\hat{f}_1$ item; (ii) hiding of the Merkle roots and authentication paths.
