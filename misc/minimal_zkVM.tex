...
\begin{itemize}
    ...
    \item Denote by $f_0$ the initial folding factor ($f_0 = 7$ in practice). About the sumcheck polynomials in the intial round ($f_0$ degree-2 polynomials): as long as there is at least 1 OOD query, and except with negligible probability, (where the OOD points would be $(0, 0, \dots )$ or ($(1, 1, \dots)$)), the first sumcheck polynomial will contain a non trivial linear combination in the random coefficients of $P(0, 1, .)$. If we call $\alpha_1$ the first sumcheck challenge, the second sucmheck polynomial will contain a non trivial linear combination from the 'folded' random coefficients of $P(0, 1, ., \alpha_1)$, etc. 
    \item The STIR querries will reveal at most $\approx 2 \cdot \lambda$ interleaved evaluations of the form $$(P(\omega, \omega^2, \omega^4, \dots, \omega^{2^{n - f_0}}, [i]_\text{bits}^{f_0}))_{i \in [0, 2^{f_0})}$$ where $\omega$ is a root-of-unity of order $\leq 2^{n-f_0}$. Important: the prover should reject the case $w = 1$ (restart the proof with another salt).
...
The public input is the hash of a field-element buffer (via the sponge of \Cref{sec:slice-hash}). The buffer is defined as follows:
