...
Strategy:
\begin{itemize}
    \item reverse variable ordering at folding, i.e. for a polynomial $P(X_1, \dots, X_n)$, fold first $X_n$, then $X_{n-1}, \dots$ (as a side note, this disables the padding-free optimization of \Cref{sec:zero-suffix}, but enables the one from \Cref{sec:svo-whir}).
    \item Store the stacked trace (i.e. the secret information) in $P(0, 0, .)$. The remaining 3/4th of the committed polynomial $P(0, 1, .), P(1, 0, .), P(1, 1, .)$ is filled with random values.
    \item $P(0, 1, .)$ is used to protect the stir + OOD queries in the initial round.
    \item $P(1, .)$ is used to add a full random mask at the end of the initial round.
\end{itemize}
...
\begin{itemize}
    \item The first few OOD evaluations (at commitment) are protected by the randomness in $P(0, 1, .)$.
    \item Denote by $k_0$ the initial folding factor ($k_0 = 7$ in practice). About the sumcheck polynomials in the intial round ($k_0$ degree-2 polynomials): as long as there is at least 1 OOD query, and except with negligible probability, (where the OOD points would be $(0, 0, \dots )$ or ($(1, 1, \dots)$)), the first sumcheck polynomial will contain a non trivial linear combination in the random coefficients of $P(0, 1, .)$. If we call $\alpha_1$ the first sumcheck challenge, the second sucmheck polynomial will contain a non trivial linear combination from the 'folded' random coefficients of $P(0, 1, ., \alpha_1)$, etc. 
    \item The STIR querries will reveal at most $\approx 2 \cdot \lambda$ interleaved evaluations of the form $$(P(\omega, \omega^2, \omega^4, \dots, \omega^{2^{n - k_0}}, [i]_\text{bits}^{k_0}))_{i \in [0, 2^{k_0})}$$ where $\omega$ is a root-of-unity of order $\leq 2^{n-k_0}$. Important: the prover should reject the case $w = 1$ (restart the proof with another salt).
    \item Sufficient requirement for $P(0, 1, .)$ to protect the sumcheck polynomials, the STIR queries and the OOD queries: the number of variables $n$ shpule be greater than $4 + \log_2 \lambda$. Trivial in practice.
    \item At the end of the initial round, $P(1, .)$ plays the role of random mask (that has been protected untill then). The folded codeword becomes totally random: zero-knowledge follows for the rest of the protocol.
\end{itemize}
