...
\textbf{3. Restart rule.} After generating the proof, the prover checks the two conditions below; if either fails, it restarts with freshly sampled masks, which re-randomizes the entire transcript. Both conditions hold except with low probability (TODO), so restarts are rare.
\begin{itemize}
...
Open questions: (i) prove the genericity lemmas: the \textsc{independence} condition holds except with low probability; (ii) hiding of the Merkle roots and authentication paths.
