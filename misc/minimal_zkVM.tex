...
WHIR \cite{whir} is the Inner Product Commitment Scheme (\Cref{sec:inner_product_PCS}) used in LeanVM. The prover commits to a vector $v \in \Fp^{2^n}$, and later proves $J$ \emph{claims} $(\hat{w}_1, c_1), \dots, (\hat{w}_J, c_J)$: each \emph{weight} $\hat{w}_j$ is the MLE of a vector $w_j \in \Fq^{2^n}$, assumed to be efficiently evaluable by the verifier, and claim $j$ states that
\[
\sum_{i=0}^{2^n - 1} v[i] \cdot w_j[i] \;=\; \sum_{b \in \{0,1\}^n} \hat{v}(b) \cdot \hat{w}_j(b) \;=\; c_j \;\in\; \Fq .
\]
...
The opening proves the $J$ input claims together with the $s_0$ out-of-domain claims of the commitment. It is parameterized by an iteration count $M$, folding factors $k_0, \dots, k_{M-1}$, domain log-reductions $v_0, \dots, v_{M-2}$ with $v_i \leq k_i$, query counts $t_0, \dots, t_{M-1}$, and out-of-domain counts $s_1, \dots, s_{M-1}$. Define $m_0 := n$, $m_{i+1} := m_i - k_i$, $\mathcal{L}_{i+1} := \mathcal{L}_i^{(2^{v_i})}$, and the folded polynomials $\hat{f}_0 := \hat{v}$, $\hat{f}_{i+1} := \hat{f}_i(\vec{\alpha}_i, \cdot)$ (where $\vec{\alpha}_i$ are the sumcheck challenges below).

The protocol maintains a \emph{running target} $\sigma \in \Fq$ and a \emph{running weight} $\hat{W}_i$: a multilinear polynomial in $m_i$ variables, defined by the batching steps below, which the verifier evaluates only once, in the final check. At the start of iteration $i$, the running claim is:
\[
\sum_{b \in \{0,1\}^{m_i}} \hat{f}_i(b) \cdot \hat{W}_i(b) \;=\; \sigma .
\]

\begin{enumerate}
    \item \textbf{Batching.} The protocol starts by batching everything into a single claim, via a random linear combination: the initial weight $\hat{W}_0$ contains both the ``useful'' input claims and the out-of-domain claims of the commitment. The verifier samples $\gamma \overset{\$}{\leftarrow} \Fq$, and all the claims are combined with successive powers of $\gamma$:
    \[
    \hat{W}_0 := \sum\nolimits_j \gamma^{j-1} \, \hat{w}_j , \qquad \sigma := \sum\nolimits_j \gamma^{j-1} \, c_j ,
    \]
    where $j$ enumerates all the claims, the $s_0$ out-of-domain claims first (each pair $(z, y)$ counting as the claim with weight $\hat{eq}(\pow(z, n), \cdot)$ and value $y$), then the input claims.
    ...
    \item \textbf{Final sumcheck ($m_M$ rounds).} As in step 2.(a), on the $m_M$ remaining variables, yielding challenges $\vec{\beta} \in \Fq^{m_M}$ and a final target $\sigma$.
...
The claimed values $c_j$ (inputs of the opening protocol) belong to the \emph{statement}: making them simulatable is the job of \Cref{sec:zk-logup-gkr,sec:zk-air-sumcheck,sec:zk-column-evals}.
...
\]
\begin{itemize}
    \item The stacking (\Cref{sec:stacking}) part (containing secret data) is unchanged, we simply use a bigger polynomial, in order to fit $R_{\textsf{in}}$ and $R_{\textsf{out}}$.
    \item $R_{\textsf{out}}$, the \emph{out-round mask}, fills the second half with i.i.d. uniform $\Fp$ cells. Since $X_1$ is folded in round 0, $R_{\textsf{out}}$ is mixed into the data from the start; its role is to protect everything revealed \emph{after} round 0.
    \item $R_{\textsf{in}}$, the \emph{in-round mask}, is a uniformly random segment occupying the \emph{last} $2^{k_0 - 1 + a}$ cells of the first half of the polynomial, with $2^a \geq t_0 + \extdeg \cdot (s_0 + s_1)$ (negligible in practice). Its role is to protect the reveals of round 0 itself: the STIR openings and the out-of-domain answers.
\end{itemize}

\textbf{3. Restart rule.} After generating the proof, the prover checks the two conditions below; if either fails, it restarts with freshly sampled masks, which re-randomizes the entire transcript. Both conditions hold except with low probability (see the proof sketch), so restarts are rare.
\begin{itemize}
...
\textbf{Overhead.} Roughly 2x.

\subsubsection{Why it is zero-knowledge (sketch)}

We go through the transcript in order.

\begin{itemize}
    \item \emph{Round-0 STIR openings.} For $s \in \{0,1\}^{k_0}$, let $u_s \in \Fp^{2^{n - k_0}}$ be the vector of cells whose round-0 coordinates equal $s$. By locality, the $2^{k_0}$ values opened by a query at $z$ contain exactly the same information as the $2^{k_0}$ evaluations $\hat{u}_s(\pow(z, n - k_0))$. The $u_s$ with $X_1$-coordinate $1$ partition $R_{\textsf{out}}$: they are entirely random. (Their revealed evaluations do consume some of $R_{\textsf{out}}$'s randomness; the $\hat{f}_1$ item below accounts for this.) Each $u_s$ with $X_1$-coordinate $0$ contains data, but ends with $2^a$ i.i.d. random cells (its share of $R_{\textsf{in}}$), and is evaluated at most $t_0$ times (duplicates reveal nothing new). On these $2^a$ random cells, $\hat{u}_s(\pow(z, \cdot))$ is the evaluation at $z$ of a univariate of degree $< 2^a$ with free coefficients, times the nonzero factor $z^{2^{n-k_0}-2^a}$. Evaluations at distinct points are independent (Vandermonde): the fiber part of \textsc{independence} holds automatically. The random cells must sit at the \emph{end} of $u_s$: elsewhere the factor contains $(1 - z^{2^i})$ terms, which vanish on small-order $z$, a constant fraction of the queries.
    \item \emph{Out-of-domain answers.} Restricted to one $u_s$, an out-of-domain answer at $z$ is a point evaluation of the same univariate, at the node $z^{2^{k_0}}$, times a factor depending on $s$, nonzero except with probability $O(\cdot)/q$. (For the $s_1$ samples on $\hat{f}_1$: node $z$, factor $\hat{eq}(\vec{\alpha}_0, s)$.) The out-of-domain points join the Vandermonde system as extra nodes; hence the size $2^a \geq t_0 + \extdeg(s_0 + s_1)$. An $\Fq$-node counts as $\extdeg$ independent linear combinations, because $\Fq$ has no proper intermediate subfield; the nodes are pairwise distinct and non-Frobenius-conjugate, except with probability $O(\cdot)/q$.
    \item \emph{Round-0 sumcheck messages.} The $2k_0$ fresh $\Fq$-coefficients are the only values of \textsc{independence} that are not point evaluations (they are partial sums). Their independence from the rest fails with probability $O(\cdot)/q$ over the verifier randomness, by Schwartz--Zippel on a nonzero minor (it helps that $s_0 \geq 1$: the OOD weight has full support, while the lifted input-claim weights vanish on the masks). Under \textsc{independence}, all the round-0 reveals are jointly uniform.
...
Open questions: (i) prove the genericity lemmas: the \textsc{independence} condition holds except with low probability; (ii) interaction of the new folding order with the optimizations of \Cref{sec:zero-suffix} and \Cref{sec:svo-whir}; (iii) hiding of the Merkle roots and authentication paths.
