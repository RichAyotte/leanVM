...
\textbf{Prover overhead.} Roughly $2\times$: the committed polynomial doubles in size.
...
\begin{itemize}
    \item \emph{Round-0 STIR openings.} For $s \in \{0,1\}^{k_0}$, let $u_s \in \Fp^{2^{n - k_0}}$ be the vector of cells whose round-0 coordinates equal $s$. By locality, a query at $z$ reveals exactly the $2^{k_0}$ evaluations $\hat{u}_s(\pow(z, n - k_0))$. The $u_s$ with $X_1$-coordinate $1$ partition $R_{\mathsf{out}}$: they are entirely random. (Their revealed evaluations do consume some of $R_{\mathsf{out}}$'s randomness; the $\hat{f}_1$ item below accounts for this.) Each $u_s$ with $X_1$-coordinate $0$ contains data, but ends with $2^a$ i.i.d. random cells (its share of $R_{\mathsf{in}}$), and is evaluated at most $t_0$ times (duplicates reveal nothing new). The contribution of these $2^a$ random cells to $\hat{u}_s(\pow(z, \cdot))$ is $z^{2^{n-k_0} - 2^a}\, g(z)$, where $g$ ranges uniformly over all univariate polynomials of degree $< 2^a$ as the random cells vary. A polynomial of degree $< 2^a$ can take any values at up to $2^a$ distinct points (a Vandermonde system), and the prefactor never vanishes: the fiber part of \textsc{independence} holds automatically.
    ...
    \item \emph{Round-0 sumcheck messages.} Notation: $z_1, z_2 \in \Fq$ are the two out-of-domain points sampled at commitment ($s_0 = 2$) and $y_1, y_2$ their answers; write $\zeta^{(j)}_i := z_j^{2^{i-1}}$ and $\vec{\zeta}^{(j)} := (\zeta^{(j)}_1, \dots, \zeta^{(j)}_{k_0})$, and the \emph{nodes} are $\nu_j := \pow(z_j^{2^{k_0}}, n - k_0)$, so that, in folding order, $\pow(z_j, n) = (\vec{\zeta}^{(j)}, \nu_j)$; the batched weight is $\hat{W}_0 = \hat{eq}(\pow(z_1, n), \cdot) + \gamma\, \hat{eq}(\pow(z_2, n), \cdot) + \gamma^2\, \hat{w}$. Round $0$ reveals $2k_0 + 2$ values: the two answers, and two \emph{fresh} coefficients per sumcheck message $\hat{h}_\ell$, $\ell = 1, \dots, k_0$ (the degree-$2$ univariate of the $\ell$-th round, written out in \Cref{sec:setting}; the third coefficient is pinned by $\hat{h}_\ell(0) + \hat{h}_\ell(1) = \sigma$). We sketch why they are jointly uniform.
    \begin{enumerate}
        \item \emph{All the messages come from one polynomial.} In the defining sum of $\hat{h}_\ell$, sum the position coordinates first: what remains is a partial boolean sum of a single polynomial $\hat{H}$ in the $k_0$ round-$0$ variables,
        \[
        ...
        \]
        \item \emph{$\hat{H}$ splits into two masked channels and a data part.} Split $\hat{W}_0$ into its three terms. In the out-of-domain ones, $\hat{eq}((\vec{\zeta}^{(j)}, \nu_j), (Y, c)) = \hat{eq}(\vec{\zeta}^{(j)}, Y)\, \hat{eq}(\nu_j, c)$, and $\sum_c \hat{f}_0(Y, c)\, \hat{eq}(\nu_j, c) = \hat{f}_0(Y, \nu_j)$, since $\hat{f}_0$ is multilinear in the position coordinates (Lagrange). With $G_j(Y) := \hat{f}_0(Y, \nu_j)$ and $\hat{H}^w(Y) := \sum_c \hat{f}_0(Y, c)\, \hat{w}(Y, c)$:
        \[
        \hat{H}(Y) \;=\; \hat{eq}\big(\vec{\zeta}^{(1)}, Y\big)\, G_1(Y) \;+\; \gamma\, \hat{eq}\big(\vec{\zeta}^{(2)}, Y\big)\, G_2(Y) \;+\; \gamma^2\, \hat{H}^w(Y) .
        \]
        The $G_j$ are multilinear in $k_0$ variables; their values on the hypercube are the \emph{node values} $G_j(s) = \hat{u}_s(\nu_j) =: V_{s,j}$; and $y_j = G_j(\vec{\zeta}^{(j)})$.
        \item \emph{The masks act on $(G_1, G_2)$ alone.} The masking argument uses only the last $2^a$ cells of each class (the \emph{blocks}). At a block position $c$, $\hat{w}(\cdot, c) = 0$ (\Cref{lem:blocks}): the blocks are absent from $\hat{H}^w$, and they act on the two other terms through their contribution to $(V_{s,1}, V_{s,2})$ only. So, whatever the size of $R_{\mathsf{in}}$, the masks control exactly the $2 \cdot 2^{k_0}$ numbers $(V_{s,j})$, that is, the pair $(G_1, G_2)$, and nothing else in $\hat{H}$.
        \item \emph{The masked part of the messages superposes two sumcheck transcripts.} By steps 1 and 2, the contribution of the channel $j$ to $\hat{h}_\ell$ is exactly the $\ell$-th message of the standard sumcheck proving the evaluation claim $G_j(\vec{\zeta}^{(j)}) = y_j$, with its weight $\hat{eq}(\vec{\zeta}^{(j)}, \cdot)$; resumming the boolean coordinates as in step 2, it equals
        \[
        \Big[ \prod_{i < \ell} \hat{eq}\big(\zeta^{(j)}_i, \alpha_i\big) \Big]\; \hat{eq}\big(\zeta^{(j)}_\ell, X\big)\; g^{(j)}_\ell(X) ,
        \]
        with
        \[
        g^{(j)}_\ell(X) \;:=\; G_j\big(\alpha_1, \dots, \alpha_{\ell-1},\, X,\, \zeta^{(j)}_{\ell+1}, \dots, \zeta^{(j)}_{k_0}\big) :
        \]
        public factors times an \emph{affine} polynomial ($G_j$ is multilinear and only $X$ is free). Directly from this definition, each transcript is a \emph{chain}:
        \[
        g^{(j)}_1\big(\zeta^{(j)}_1\big) \;=\; y_j , \quad g^{(j)}_{\ell+1}\big(\zeta^{(j)}_{\ell+1}\big) \;=\; g^{(j)}_\ell(\alpha_\ell) , \quad g^{(j)}_{k_0}(\alpha_{k_0}) \;=\; G_j(\vec{\alpha}_0) \;=\; \hat{f}_1(\nu_j) ,
        \]
        the last relation being the exit: it pins the node evaluations of $\hat{f}_1$, taken up in the next item. A chained affine carries one fresh dimension, so the masked content of the round-0 view is $y_1, y_2$ plus two fresh dimensions per round: $2k_0 + 2$, exactly the number of revealed values. (With a single point, the same count gives $k_0 + 1$ masked dimensions for $2k_0 + 1$ values, and the $k_0$ missing directions show the data, whatever the size of the masks: \Cref{prop:rank,rem:identities}. This is why $s_0 \geq 2$.)
        \item \emph{Full rank, generically.} Each message shows only the $\gamma$-weighted \emph{sum} of the two chains' contributions, so it remains to check that no combination of the $2k_0 + 2$ revealed values escapes both chains: they are jointly independent functionals of the $(V_{s,j})$, proved in \Cref{thm:twopoint} under per-slot conditions ($\zeta^{(1)}_\ell \neq \zeta^{(2)}_\ell$ and $(2\zeta^{(1)}_\ell - 1)(2\zeta^{(2)}_\ell - 1) \neq -1$) failing with probability at most $(4 \cdot 2^{k_0} + 2k_0)/q$ (\Cref{cor:twopointprob}; verified numerically: rank $16/16$ at $k_0 = 7$). Finally, the blocks realize any prescribed $(V_{s,1}, V_{s,2})$ jointly with the queried-point evaluations (Chinese remaindering, \Cref{lem:crt}): under \textsc{independence}, the round-0 reveals are jointly uniform.
    \end{enumerate}
    \item \emph{The folded polynomial $\hat{f}_1$.} Each coefficient of $\hat{f}_1$ mixes data and mask:
    \[
    \hat{f}_1[c] \;=\; \big(\text{combination of data cells}\big) \;+ \sum_{s' \in \{0,1\}^{k_0 - 1}} \hat{eq}\big(\vec{\alpha}_0, (1, s')\big)\, R_{\mathsf{out}}[(c, s')].
    \]
    The random cells on the right are disjoint across $c$, with weights independent of $c$. If these $2^{k_0 - 1}$ weights span $\Fq$ over $\Fp$, which is exactly the \textsc{spread} condition, then every coefficient is uniform, independently: $\hat{f}_1$ is a uniformly random polynomial. (\textsc{spread} fails with probability at most $2/q$: \Cref{lem:spread} below.) One subtlety: round 0 already revealed information about $R_{\mathsf{out}}$, so conditioned on the view, $\hat{f}_1$ is no longer fully uniform. The leak has a narrow structure. Fibers and out-of-domain answers touch a mask class only through its evaluations at the \emph{nodes} (the queried points and the out-of-domain nodes), and the sumcheck messages add at most $2k_0$ further combinations: condition on all of them. Each mask class then remains uniform on the subspace $V \subseteq \Fp^{2^{n - k_0}}$ of vectors vanishing at every node, the \emph{same} subspace for every class, cut by the few sumcheck combinations. Under \textsc{spread}, the mix of these $2^{k_0 - 1}$ independent residuals in the display above is uniform on a subspace $W$ of small explicit codimension ($W$ is the $\Fq$-span of $V$, cut by those combinations: the mix is an $\Fp$-linear surjection onto the span, and a surjective linear image of a uniform variable is uniform). So, given the view, $\hat{f}_1$ is uniform on an explicit coset of $W$: every direction outside the annihilator of $W$ (the combinations vanishing on $W$) is perfectly masked. The annihilator is spanned by the evaluations at the nodes and the few inherited sumcheck directions, and on each of its directions the conditional value of $\hat{f}_1$ is a function of the view, which the simulator generates itself. Among these pinned directions sit the constraints the protocol checks anyway: $\hat{f}_1(\pow(z_t, n - k_0)) = \fold(f_0, \vec{\alpha}_0)(z_t)$ for each queried $z_t$; the $s_1$ out-of-domain answers; and the running sumcheck claim $\sum_b \hat{f}_1(b)\, \hat{W}_0(\vec{\alpha}_0, b) = \sigma$, with $\sigma$ the target at the end of the round-0 sumcheck. (The list is not exhaustive: the evaluations of $\hat{f}_1$ at the commitment nodes are also pinned, their node combinations lying in the span of the revealed values by \Cref{thm:twopoint}. Pinned directions are harmless: their values are computable from the view.)
...
Open question: the BCS transform. \Cref{cor:statzk} proves the \emph{interactive} protocol statistically zero-knowledge against the honest verifier, at distance $\approx 2^{-122}$, with no prover-side check.

\section{Proofs for the masked WHIR}

We analyse the linear algebra of the values revealed during round 0 of the masked opening, and prove the lemmas behind the \textsc{independence} condition of \Cref{sec:zk-sketch}. The route: the two-point system of \Cref{sec:twopoint} splits per point, each point contributing its own block of node values, so \Cref{sec:slot,sec:rank} first work out the structure contributed by \emph{one} point, with an explicit rank and kernel (\Cref{prop:rank}); \Cref{sec:twopoint} then assembles the two blocks into joint independence, under explicit per-slot conditions (\Cref{thm:twopoint}; stated at $s_0 = 2$, the case we instantiate, while \Cref{rem:s0} treats $s_0 > 2$). The cross-freeness of the blocks (\Cref{lem:blocks}) then upgrades independence to the joint uniformity of the full round-0 view (\Cref{prop:uniform}), and \Cref{sec:simulator} builds the simulator: perfect simulation on an explicit good set of challenge tuples, whose complement is bounded (\Cref{cor:restart}) and absorbed into the statistical error (\Cref{cor:statzk}).

\subsection{Setting and notation}\label{sec:setting}

The two-point analysis of \Cref{sec:twopoint} proceeds by restricting the revealed values to the block of one point at a time, and each such restriction is a one-point computation: \Cref{sec:slot,sec:rank} carry it out. (Nothing here concerns an $s_0 = 1$ variant of the protocol.) For a point $z$, write $\zeta_i := z^{2^{i-1}}$, so that $\pow(z, n) = (\zeta_1, \dots, \zeta_{k_0}, \pow(z^{2^{k_0}}, n - k_0))$. The \emph{node} is the point $\nu := \pow(z^{2^{k_0}}, n - k_0)$, and the \emph{node values} are
\[
V_s \;:=\; \hat{u}_s(\nu) \;\in\; \Fq , \qquad s \in \{0,1\}^{k_0} ,
\]
with $u_s$ the classes of \Cref{sec:zk-sketch}. We compute with the one-point weight $\hat{W}_0 = \hat{eq}(\pow(z, n), \cdot) + \gamma\, \hat{w}$; the two-point batching scalars are restored in \Cref{sec:twopoint}. The input weight $\hat{w}$ vanishes on every mask cell. The round-0 sumcheck messages are
\[
...
\]
Then the $2k_0 + 2$ value rows are linearly independent. Moreover, their span contains the two extension rows $\eta_j$ (coefficient $\hat{eq}(\vec{\alpha}_0, s)$ at column $(s,j)$, supported on block $j$): the node combinations giving the evaluations of $\hat{f}_1$ at the two nodes are determined by the revealed values. In the $\hat{f}_1$ analysis of \Cref{sec:zk-sketch}, these evaluations accordingly join the directions pinned by the view; this is harmless, since pinned values are functions of the view, which the simulator generates itself.
\end{theorem}
