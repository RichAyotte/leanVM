...
\textbf{3. Out-of-domain count.} We require $s_0 \geq 2$ out-of-domain points at commitment. This is a zero-knowledge requirement, not necessarily a soundness one: with $s_0 = 1$, the round-0 sumcheck messages can never be masked, whatever the size of the masks (see the proof sketch).
...
A challenge tuple is outside the good set only if one of the following fails: \textsc{spread} (at most $2/q$, \Cref{lem:spread}); the conditions of \Cref{prop:uniform} (at most $(4 \cdot 2^{k_0} + 2k_0)/q$ for \Cref{thm:twopoint}, plus $(s_0 + s_1)\, p/q$ for a node landing in $\Fp$ by the exact count of \Cref{sec:zk-sketch}, plus $\binom{s_0 + s_1}{2} \extdeg\, 2^{k_0}/q$ for collisions and conjugations); the twist-rigidity assumptions (at most $p/q$ plus a negligible term, \Cref{cor:twistprob}); the cross coupling (at most $2^{k_0+7}/q$, \Cref{cond:cross2}, at the $\Fq$ level: \Cref{rem:slice}). Total, for KoalaBear with $s_0 = 2$:
\[
