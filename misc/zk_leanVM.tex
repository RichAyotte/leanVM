...
We analyse the linear algebra of the values revealed during round 0 of the masked opening, and prove the lemmas behind the \textsc{independence} condition of \Cref{sec:zk-sketch}. The route: \Cref{sec:slot,sec:rank} work at a single out-of-domain point and compute the revealed values as linear forms in the per-class node values, with an explicit rank and kernel (\Cref{prop:rank}); \Cref{sec:twopoint} assembles two points into joint independence, under explicit per-slot conditions (\Cref{thm:twopoint}; stated at $s_0 = 2$, the case we instantiate, while \Cref{rem:s0} treats $s_0 > 2$). The cross-freeness of the blocks (\Cref{lem:blocks}) then upgrades independence to the joint uniformity of the full round-0 view (\Cref{prop:uniform}), and \Cref{sec:simulator} builds the simulator: perfect simulation on an explicit good set of challenge tuples, whose complement is bounded (\Cref{cor:restart}) and absorbed into the statistical error (\Cref{cor:statzk}).
