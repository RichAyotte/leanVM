...
\paragraph{Row computation.} Each message $\hat{h}_\ell$ is a linear combination of the committed cells $P[u]$ with coefficients in $\Fq[X]$: expanding $\hat{f}_0 = \sum_u P[u] \hat{eq}(u, \cdot)$ in its definition, the coefficient of $P[u]$, for $u \in \{0,1\}^n$, is the degree-$2$ polynomial
\[
...
\end{equation}
Summing up: as a linear form on the cells, every revealed value $v$ (an out-of-domain answer, or a fresh coefficient of a sumcheck message $\hat{h}_\ell$) is the sum of three parts. The out-of-domain term of $\hat{W}_0$ contributes the \emph{node component} $N_v$, a linear form in the node values $(V_s)_s$. The $\hat{w}$ term vanishes on $R_{\mathsf{in}}$ (\Cref{lem:blocks} below) and contributes a form supported on the data cells and on $R_{\mathsf{out}}$; the $R_{\mathsf{out}}$ part is the \emph{cross component} $C_v$, and its contributions are called the \emph{cross-terms}.
