...
\end{itemize}
The simulator theorem (\Cref{thm:simulator}) adds two further checks of the same character, twist rigidity and cross coupling (\Cref{cond:twist,cond:cross2}), public and prover-checkable. All checks are bounded analytically: $\leq 2/q$ for \textsc{spread} (\Cref{lem:spread}); $\leq (4 \cdot 2^{k_0} + 2k_0)/q$ for the two-point rank conditions (\Cref{cor:twopointprob}); $\leq (2 \cdot 2^{k_0} + s_1)\, p/q$ for a node landing in $\Fp$ (the dominant term); $\leq \binom{s_0 + s_1}{2} \extdeg\, 2^{k_0}/q$ for node collisions or conjugations; $\leq p/q$ plus negligible for twist rigidity (\Cref{cor:twistprob}); $\leq 2^{k_0+6}/q$ for cross coupling (\Cref{cond:cross2}). Total:
\[
...
\subsection{Why it is (perfect) zero-knowledge (sketch)}\label{sec:zk-sketch}
...
Open question: the Fiat--Shamir and Merkle layers. \Cref{thm:simulator} proves the \emph{interactive} protocol perfectly zero-knowledge against the honest verifier, with all restart conditions bounded ($\Pr[\text{restart}] \approx 2^{-116}$).
...
\begin{corollary}[restart probability]\label{cor:restart}
The prover restarts only if one of the following fails: \textsc{spread} (at most $2/q$, \Cref{lem:spread}); the conditions of \Cref{prop:uniform} (at most $(4 \cdot 2^{k_0} + 2k_0)/q$ for \Cref{thm:twopoint}, plus $(2 \cdot 2^{k_0} + s_1)\, p/q$ for a node landing in $\Fp$, plus $\binom{s_0 + s_1}{2} \extdeg\, 2^{k_0}/q$ for collisions and conjugations); $\alpha_1 \in \Fp$ and the rank of \Cref{cor:twistprob} (at most $p/q$ plus a negligible term); the cross coupling (at most $2^{k_0+6}/q$). Total, for KoalaBear with $s_0 = 2$:
\[
\Pr[\mathrm{restart}] \;\leq\; \big(2 \cdot 2^{k_0} + s_1 + 1\big)\, \frac{p}{q} \;+\; O\big(2^{k_0 + 6}\big)\frac{1}{q} \;\approx\; 2^{-116} ,
\]
