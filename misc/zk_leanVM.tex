...
\[
\Pr[\text{degenerate challenges}] \;\leq\; \big(s_0 + s_1 + 1\big)\, \frac{p}{q} \;+\; O\big(2^{k_0+6}\big)\frac{1}{q} \;\approx\; 2^{-122} \quad \text{for KoalaBear}
\]
...
\end{itemize}
(All conditions are public, so the prover \emph{could} check them and restart with fresh masks on failure, upgrading statistical to perfect zero-knowledge. We do not require it: the checks are nontrivial to implement, and $2^{-122} \leq 2^{-\lambda}$ for the targets in scope. A cheaper middle ground reaches $\lambda = 128$: the four dominant events are single-challenge public conditions, namely $z_1^{2^{k_0}} \in \Fp$, $z_2^{2^{k_0}} \in \Fp$, an $\hat{f}_1$ point in $\Fp$, and $\alpha_1 \in \Fp$, so resampling just these challenges on violation, deterministically on both sides, leaves a total of order $2^{k_0+6}/q \approx 2^{-141}$.)
...
\begin{itemize}
    ...
    \item \emph{Out-of-domain answers.} Restricted to one $u_s$, an out-of-domain answer at $z$ is a point evaluation of the same univariate, at the node $z^{2^{k_0}}$, times a factor depending on $s$, nonzero unless $z = 0$ or $z^{2^{k_0 - 1}} = 1$: probability at most $2^{k_0}/q$. (For the $s_1$ samples on $\hat{f}_1$: node $z$, factor $\hat{eq}(\vec{\alpha}_0, s)$, nonzero unless a challenge lies in $\{0, 1\}$: at most $2 k_0/q$.) The out-of-domain points join the Vandermonde system as extra nodes; hence the size $2^a \geq t_0 + \extdeg(s_0 + s_1)$. An $\Fq$-node counts as $\extdeg$ independent linear combinations, because $\Fq$ has no proper intermediate subfield, provided the node lies outside $\Fp$: this fails with probability exactly $p/q$ per node. (For a commitment node: the set $\{z : z^{2^{k_0}} \in \Fp\}$ has size exactly $p$, since the $2^{k_0}$-th powers inside $\Fp^*$ form the subgroup of order $(p-1)/2^{k_0}$, using $2^{k_0} \mid p-1$, and each has $2^{k_0}$ preimages.) These events, of order $p/q \approx 2^{-124}$ for KoalaBear, dominate the total. Finally, the nodes must be pairwise distinct and non-Frobenius-conjugate: at most $\extdeg \, 2^{k_0}/q$ per pair (an orbit has $\extdeg$ elements, each with at most $2^{k_0}$ preimages).
...
Open question: the Fiat--Shamir and Merkle layers. \Cref{cor:statzk} proves the \emph{interactive} protocol statistically zero-knowledge against the honest verifier, at distance $\approx 2^{-122}$, with no prover-side check.
...
For uniform challenges (the interactive setting), and for every data assignment $D$, the honest transcript with data $D$ is at statistical distance at most $\Pr[\text{degenerate challenges}] \approx 2^{-122}$ from the output of the simulator (the honest prover run on $D^*$), with no prover-side check.
\end{corollary}
...
A challenge tuple is outside the good set only if one of the following fails: \textsc{spread} (at most $2/q$, \Cref{lem:spread}); the conditions of \Cref{prop:uniform} (at most $(4 \cdot 2^{k_0} + 2k_0)/q$ for \Cref{thm:twopoint}, plus $(s_0 + s_1)\, p/q$ for a node landing in $\Fp$ by the exact count of \Cref{sec:zk-sketch}, plus $\binom{s_0 + s_1}{2} \extdeg\, 2^{k_0}/q$ for collisions and conjugations); the twist-rigidity assumptions (at most $p/q$ plus a negligible term, \Cref{cor:twistprob}); the cross coupling (at most $2^{k_0+6}/q$, \Cref{cond:cross2}). Total, for KoalaBear with $s_0 = 2$:
\[
\Pr[\mathrm{degenerate\ challenges}] \;\leq\; \big(s_0 + s_1 + 1\big)\, \frac{p}{q} \;+\; O\big(2^{k_0 + 6}\big)\frac{1}{q} \;\approx\; 2^{-122} ,
\]
