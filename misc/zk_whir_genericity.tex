...
\section{Bounding the pinned directions}\label{sec:pinbound}
...
The prover restarts only if one of the following fails: \textsc{spread} (at most $2/q$, main document); the conditions of \Cref{prop:uniform} (at most $(4 \cdot 2^{k_0} + 2k_0)/q$ for \Cref{thm:twopoint}, plus $(2 \cdot 2^{k_0} + s_1)\, p/q$ for a node landing in $\Fp$, plus $\binom{s_0 + s_1}{2} \extdeg\, 2^{k_0}/q$ for collisions and conjugations); $\alpha_1 \in \Fp$ and the rank of \Cref{cor:twistprob} (at most $p/q$ plus a negligible term); the cross coupling (at most $2^{k_0+6}/q$). Total, for KoalaBear with $s_0 = 2$:
\[
