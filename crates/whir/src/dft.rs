...
fn par_initial_layers<F: Field>(mat: &mut [F], chunk_size: usize, root_table: &[Vec<F>], width: usize) {
    parallel::par_chunks_mut(mat, chunk_size, |_, chunk| {
...
fn dft_layer_par<F: Field, B: Butterfly<F>>(vec: &mut [F], twiddles: &[B], width: usize) {
    parallel::par_chunks_mut(vec, twiddles.len() * 2 * width, |_, block| {
        let (left, right) = block.split_at_mut(twiddles.len() * width);
        for ((hi_chunk, lo_chunk), twiddle) in left
...
fn dft_layer_par_double<F: Field, B: Butterfly<F>, M: MultiLayerButterfly<F, B>>(
    ...
    parallel::par_chunks_mut(&mut *mat.values, twiddles_large.len() * 2 * width, |_, block| {
        ...
        for (ind, (((hi_hi, hi_lo), lo_hi), lo_lo)) in hi_hi_blocks
            .chunks_exact_mut(width)
...
fn dft_layer_par_triple<F: Field, B: Butterfly<F>, M: MultiLayerButterfly<F, B>>(
    ...
    parallel::par_chunks_mut(&mut *mat.values, twiddles_large.len() * 2 * width, |_, block| {
        ...
        for (ind, (((((((hi_hi_hi, hi_hi_lo), hi_lo_hi), hi_lo_lo), lo_hi_hi), lo_hi_lo), lo_lo_hi), lo_lo_lo)) in
            hi_hi_hi_blocks
                .chunks_exact_mut(width)
