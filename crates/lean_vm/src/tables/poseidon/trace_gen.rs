...
// columns: this pass recomputes them from the (final) input columns, 8 rows per
// packed call, parallel over disjoint row ranges. Trailing rows that don't fill
// a packed block — and every row on targets without the packed kernel — use the
// scalar replay, which emits identical values.
...
// Rows are addressed by index across 8 input columns and the raw aux column
