...
impl PrimeCharacteristicRing for Goldilocks {
    ...
    //   each term is < 2^128, the accumulator bound caps terms at < 2^23, and
    ...
    #[inline]
    fn lazy_acc_finish(acc: [Self; 4], _n_sub: u64) -> Self {
        // Subtraction above is exact (borrow chains); no deficit to repay.
        let l0 = acc[0].value;
        let l1 = acc[1].value;
        let l2 = acc[2].value;
        // l2 drifts by <= 1 per term around its ~P/4 ~ 2^62 seed.
        debug_assert!(l2 < (1u64 << 63), "lazy accumulator l2 overflow (too many terms)");
