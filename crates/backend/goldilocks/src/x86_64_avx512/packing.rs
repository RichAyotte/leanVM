...
impl PrimeCharacteristicRing for PackedGoldilocksAVX512 {
    ...
    // sub_no_double/add_no_double tail as `reduce128` below. Accumulation is
    // capped at < 2^23 terms, so W, K < 2^23 and s = H'_hi + W*2^32 < P.
            ...
            #[cfg(debug_assertions)]
            {
                ...
                for i in 0..WIDTH {
                    debug_assert!(
                        ...
                        "lazy accumulator wrap counters overflow (too many terms)"
                    );
                }
            }
