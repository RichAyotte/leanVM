...
use crate::{SumcheckComputation, packing_decompose, sumcheck_prove_many_rounds};
...
pub fn run_product_sumcheck<EF: ExtensionField<PF<EF>>>(
    ...
    if let (MleRef::BasePacked(evals), MleRef::ExtensionPacked(weights)) = (pol_a, pol_b) {
        if EF::DIMENSION == 3 {
            // Lazy path for large instances only: 18 variables is the empirical
            // crossover where deferred reduction beats the eager path; below it
            // (e.g. bytecode_claims' small domains) the eager path is faster.
...
/// Horizontal sum of a packed field element's lanes.
fn lane_sum<F: PrimeField64, PF: PackedField<Scalar = F>>(p: PF) -> F {
    let mut s = F::ZERO;
    for &v in p.as_slice() {
        s += v;
    }
...
/// Eager path for the (BasePacked, ExtensionPacked) arm, extracted from
/// `run_product_sumcheck` as the fallback for small instances. Generic over
/// `DIM` so it stays exercisable for extension fields other than the cubic one.
pub fn run_product_sumcheck_base_eager<const DIM: usize, EF: ExtensionField<PF<EF>>>(
    ...
    let (second_sumcheck_poly, folded) =
        fold_and_compute_product_sumcheck_polynomial(evals, weights, r1, sum, packing_decompose::<EF>);
...
/// transcript is bit-identical to the eager path.
pub fn run_product_sumcheck_base_lazy<const DIM: usize, EF: ExtensionField<PF<EF>>>(
    ...
    let decompose = packing_decompose::<EF>;
...
/// work is five passes (W'-fold + one per stream), each holding only DIM lazy
/// accumulators to stay within the SIMD register budget.
...
>(
    ...
    // Four unreduced streams (n_sub = 0: every term is a raw positive product of
    // canonical elements; chunk length 1024 stays under the accumulator bound).
...
>(
    ...
    let (c0_acc, c2_acc) = parallel::map_reduce(
        ...
        |chunk| {
            ...
            // Two passes over the (L2-resident) chunk: keeping both c0 and c2
            // accumulators live at once (6 groups x 4 SIMD regs) spills, so we
            // split into two passes of DIM accumulators each.
