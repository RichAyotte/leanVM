...
pub struct SingleMessageInfo {
    pub without_pubkeys: SingleMessageInfoWithoutPubkeys,
...
impl<'de> Deserialize<'de> for SingleMessageInfo {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        let (without_pubkeys, pubkeys) = <(SingleMessageInfoWithoutPubkeys, Vec<XmssPublicKey>)>::deserialize(d)?;
...
impl SingleMessageAggregateSignature {
    ...
    pub fn decompress_without_pubkeys(bytes: &[u8], pubkeys: Vec<XmssPublicKey>) -> Option<Self> {
        let (without_pubkeys, proof) = lz4_postcard_decode::<(SingleMessageInfoWithoutPubkeys, ExecutionProof)>(bytes)?;
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SingleMessageInfoWithoutPubkeys {
    pub message: [u8; MESSAGE_LENGTH],
    pub slot: u32,
    pub bytecode_claim: Evaluation<EF>,
}

impl Serialize for SingleMessageInfoWithoutPubkeys {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        (&self.message, &self.slot, &self.bytecode_claim.point).serialize(s)
    }
}

impl<'de> Deserialize<'de> for SingleMessageInfoWithoutPubkeys {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
...
impl SingleMessageInfoWithoutPubkeys {
    pub(crate) fn with_pubkeys(self, mut pubkeys: Vec<XmssPublicKey>) -> Option<SingleMessageInfo> {
        pubkeys.sort();
        pubkeys.dedup();
        Some(SingleMessageInfo {
...
impl SingleMessageInfo {
    ...
    pub fn decompress_without_pubkeys(bytes: &[u8], pubkeys: Vec<XmssPublicKey>) -> Option<Self> {
        lz4_postcard_decode::<SingleMessageInfoWithoutPubkeys>(bytes)?.with_pubkeys(pubkeys)
    }
...
/// Single-message aggregation = one message, one slot.
#[instrument(skip_all)]
pub fn aggregate_single_message_signatures(
    ...
    aggregate_single_message_with_min_padding(children, raw_xmss, message, slot, log_inv_rate, BTreeMap::new())
}

pub(crate) fn aggregate_single_message_with_min_padding(
    children: &[SingleMessageAggregateSignature],
    ...
    // Standard single-message (not a split).
    ...
    Ok(SingleMessageAggregateSignature {
        info: SingleMessageInfo {
            without_pubkeys: SingleMessageInfoWithoutPubkeys {
                message: *message,
                slot,
...
mod tests {
    ...
    fn test_recursive_aggregation_extension_table_bigger_than_execution() {
        ...
        let inner =
            aggregate_single_message_with_min_padding(&[], raws_inner, message, slot, log_inv_rate, min_padding)
                .unwrap();
