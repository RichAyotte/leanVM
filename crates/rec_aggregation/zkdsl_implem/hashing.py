...
def slice_hash_rtl(data, num_chunks, iv):
    ...
    poseidon8_compress(iv, data + (num_chunks - 1) * DIGEST_LEN, states)
...
def slice_hash_range(data, num_chunks, dest):
    ...
    poseidon8_compress(iv, data, states)
...
def absorb_n_hashes_const(n: Const, sp_in, dp_in):
    ...
    for _ in unroll(0, n):
        ...
        poseidon8_compress(sp, dp, new_state)
        sp = new_state
        dp += DIGEST_LEN
    return sp
...
def slice_hash_runtime(data, num_chunks):
    ...
    if num_chunks == 1:
        ...
        poseidon8_compress(iv, data, result)
        return result
