...
def batch_hash_slice_ltr_with_iv(num_queries, all_data_to_hash, all_resulting_hashes, num_chunks):
    if num_chunks == DIM * 2:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, DIM * 2)
        return
    if num_chunks == 16:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, 16)
        return
    if num_chunks == 8:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, 8)
        return
    if num_chunks == 20:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, 20)
        return
    if num_chunks == 1:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, 1)
        return
    if num_chunks == 4:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, 4)
        return
    if num_chunks == 5:
        batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, 5)
        return
    print(num_chunks)
    assert False, "batch_hash_slice called with unsupported len"


def batch_hash_slice_ltr_const(num_queries, all_data_to_hash, all_resulting_hashes, num_chunks: Const):
    iv = build_iv(num_chunks * DIGEST_LEN)
    for i in range(0, num_queries):
        data = all_data_to_hash[i]
        res = slice_hash_iv(data, num_chunks, iv)
...
@inline
def slice_hash_iv(data, num_chunks, iv):
    ...
    for j in unroll(1, num_chunks):
        poseidon16_compress(
            states + (j - 1) * DIGEST_LEN, data + j * DIGEST_LEN, states + j * DIGEST_LEN
        )
